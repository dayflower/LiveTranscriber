#!/bin/bash
# Submits build/LiveTranscriber.app to Apple's notary service and staples the
...
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
ditto -c -k --sequesterRsrc --keepParent "$app" "$tmp/LiveTranscriber.zip"
...
xcrun notarytool submit "$tmp/LiveTranscriber.zip" \
	--key "$NOTARY_KEY_PATH" \
